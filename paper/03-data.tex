...
%ranging from \textquote{Assisted suicide should be a criminal offence.} to 
%\textquote{We should abandon marriage.}, therefore drawing a clear line between supporting and non-supporting key points.
...
Many of these words belong to the set of stop words, such as \textquote{the}, \textquote{to}, \textquote{is}, \textquote{of}, \textquote{be}, \textquote{a}, \textquote{are}, \textquote{in}, \textquote{should}, \textquote{for}, \textquote{can}, \textquote{not}.
\todo{Less example words.}
...
In the first example pair (Table \ref{tab:data-example}), the key point can be matched with its argument. Both sentences were written about children actors and their education. The word \textquote{actors} is not explicitly used in the key point but is semantically similar to the word \textquote{performer}. 
...
It is recognized that the meaning of sentences understanding can be a challenge for the task. In the second example, the argument and the key point were expressed differently. There are two negations \textquote{not fair to not allow} and additionally a restriction with \textquote{as long as} for the main clause before used in the argument. 
