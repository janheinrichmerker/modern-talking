...
For example, the words \textquote{assistance} and \textquote{assisted} from the above example~(Table~\ref{examples},~C) are both stemmed to \textquote{assist} with the Snowball stemmer. 
Consequently, stemming creates an overlap between different forms of the same word and, for instance, increases the probability that an argument containing \textquote{assistance} be associated to a key point containing \textquote{assisted}.
