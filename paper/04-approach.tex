...
To compute similarity between an argument and a key point, let~\(\text{tokens}_a\) be the set of tokens from an argument~\(a\) and \(\text{tokens}_k\)~the set of tokens from a key point~\(k\).
...
To improve upon the simple token overlap baseline, we fine-tune \Bert and \Roberta language models for classifying argument key point matches~\cite{DevlinCLT2019,LiuOGDJCLLZS2019}.
