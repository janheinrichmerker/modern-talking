...
\end{figure}
For example, the argument and key point shown in Figure~\ref{example-overlap} both contain the token \textquote{suffering}.
...
For example, the word \textquote{weakness} will be stemmed to \textquote{weak} with the Snowball stemmer. 
Consequently, stemming creates an overlap between different forms of the same word and, for instance, increases the probability that an argument containing \textquote{weakness} be associated to a key point containing \textquote{weak}.
...
To improve upon the simple token overlap baseline, we fine-tune \Bert and \Roberta Transformer models for classifying argument key point matches~\cite{DevlinCLT2019,LiuOGDJCLLZS2019}.
