...
def get_predictions(predictions_file, labels_df, arg_df):
    ...
    predictions_df = pd.merge(arg_df, predictions_df,
                              how="left", on="arg_id")

    # handle arguements with no matching key point
    predictions_df["key_point_id"] = predictions_df["key_point_id"] \
        .fillna("dummy_id")
    predictions_df["score"] = predictions_df["score"].fillna(0)

    # merge each argument with the gold labels
    merged_df = pd.merge(predictions_df, labels_df,
                         how="left", on=["arg_id", "key_point_id"])
...
def evaluate(gold_data_dir, predictions_file):
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("You must specify two parameters for this scripts: input data "
              "directory and the predictions file")
    else:
